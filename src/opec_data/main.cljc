(ns opec_data.main
  "Kotodama WASM entrypoint for the Opec_data clean-room actor (L4) — Clojure port.
  ...
  facts (`opec_data.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "opec_data")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"opec_data.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "opec_data-compat" :tier tier :entities entities} 200])
...
(defn start! [] :opec_data-compat/ready)
